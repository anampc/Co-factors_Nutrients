...
# Data
    ...
    Temp<-Temp %>% mutate(Group =
                     case_when(Tank == "Tank.1" ~ "Control Temp", 
                               Tank == "Tank.2" ~ "High Temp", 
                               Tank == "Tank.3" ~ "High Temp", 
                               Tank == "Tank.4" ~ "Control Temp",
                               Tank == "Tank.5" ~ "Control Temp", 
                               Tank == "Tank.6" ~ "Donor colony", 
                               Tank == "Tank.7" ~ "Donor colony", 
                               Tank == "Tank.8" ~ "High Temp",))
    Temp<-Temp[Temp$Temperature>27.5,]
    summary(Temp)
```
  ...
  scale_y_continuous(limits = c(26, 32),
    ...
    scale_x_datetime(breaks = "5 days",
                      ...
                      limits = c(as.POSIXct("2023-05-02 12:30"),
                                 as.POSIXct("2023-05-18 02:00")),
                      #  expand = c(0.01, 0.01),
                      name=("Date")) +
  # geom_vline(xintercept = as.POSIXct("2023-05-18"),
  #            linetype=3)+
   # geom_vline(xintercept = as.POSIXct("2023-05-13"),
   #           linetype=3)+
  ggthe_bw + facet_wrap(~Tank, ncol = 4)+
  #facet_wrap(~(fct_rev(Tank)), ncol = 4)+
  scale_colour_manual(
  values = c("#2b83ba",  "gray", "#d7191c"))+
...
```{r}
# Data
    data<-read.csv("Data/Samples.csv", header = TRUE)
    data<-data[data$Sample.ID<833,]
    summary(data)
    ...
    data$Treatment<-factor(data$Treatment, levels = c ("LN_28", "LN_31", "HN_28", "HN_31", "Healthy", "Disease",  "Stock", "VAT"))
...
```{r}
NH4<- ggplot(data, aes (Target, NH4.concentration..uM.)) +
  #geom_boxplot ()+
  ...
  ggthe_bw +
  facet_grid(Beaker.conditions~TimePoint, scales = "free_y")
...
```{r}
NH4<- ggplot(data, aes (TimePoint, NH4.concentration..uM.)) +
  geom_boxplot ()+
...
```{r}
NH4_Arms<- ggplot(data, aes (TimePoint, NH4.concentration..uM.)) +
  geom_boxplot ()+
...
```{r}
NH4_Temp<- ggplot(data, aes (TimePoint, NH4.concentration..uM., fill=Treatment)) +
...
```{r}
Concentrations <- ddply (data, .(Treatment, TimePoint),
                         summarise,
                meanNH4 = mean (Observed.concentration..uM., na.rm = T), 
                sdNH4 = sd (Observed.concentration..uM., na.rm = T))
Concentrations

Concentrations<-Concentrations %>% 
 mutate_if(is.numeric, round, digits=2)

Concentrations<-reshape(Concentrations, idvar = c("TimePoint"),
...
```{r}
NH4<- ggplot(data, aes (Treatment, Observed.concentration..uM.)) +
  geom_boxplot ()+
