...
Temperature<- ggplot(Temp, aes (Time, Temperature)) +
  ...
  scale_y_continuous(limits = c(26, 32),
    ...
    scale_x_datetime(breaks = "14 days",
                      date_labels = "%b %d",
...
```{r}
NH4_2<- ggplot(data, aes (TimePoint, NH4)) +
  geom_boxplot ()+
...
```{r}
NH4_Arms<- ggplot(data, aes (TimePoint, NH4)) +
  geom_boxplot ()+
...
```{r}
NH4_Temp<- ggplot(data, aes (Date, NH4, fill=Treatment)) +
