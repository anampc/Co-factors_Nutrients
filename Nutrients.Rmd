...
Temperature<- ggplot(Temp, aes (Time, Temperature)) +
  ...
  geom_vline(xintercept = as.POSIXct("2023-05-18"),
              linetype=2)+
  annotate("text", x = c(as.POSIXct("2023-05-13"), as.POSIXct("2023-06-02")), y = 27, label = "*")+
...
Temp.stats$day<-as.factor(Temp.stats$day)

Temp.stats1<-Temp.stats %>% group_by("day", "Tank") %>%
...
```{r}
NH4_2<- ggplot(data, aes (Date, NH4)) +
  geom_boxplot ()+
